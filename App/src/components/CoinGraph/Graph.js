...
const CoinGraph = ({ match }) => {
	...
	return (
		...
		<div>
			...
			{console.log(values)}
			<div className="description"> The coin {values.id} currently has a coin rank of {values.coingecko_rank} and a market cap rank of {values.market_cap_rank}. It uses a {values.hashing_algorithm} hashing algorithm, and its block time in minutes is {values.block_time_in_minutes}.</div>
		</div>
	);
};
